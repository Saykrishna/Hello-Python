...
import logging
import C04b_OOPDBGUI3tier.p01_1da_sales_db as db
from C04b_OOPDBGUI3tier.p01_1da_sales import Sales, Regions
    ...
    def get_amount(self):
        ...
        if sales_date == "" or region_code == "":
            ...
            try:
                ...
                # check if region is one of the right options
                ...
                if region_code not in region_codes:
                    messagebox.showerror("Error", f"{region_code} is not one of the following \n"
                                                  f"region codes: {region_codes}")
                else:  # check if there are sales for the specified date and region
